...
export class AudioEngine {
  ...
  public updateParameters(index: number) {
    ...
    // Only update master gain here if it's not being set directly via setVolume
    if (!this._volumeBeingSet) {
      const normalizedVolume = state.level / 100;
      node.masterGain.gain.value = Math.min(normalizedVolume, 1);
    }
  }
  ...
  public setVolume(volume: number) {
    ...
    this.state[this.activeTab].level = volume;

    // Use exponential ramping for smoother volume changes
    this.nodes[this.activeTab].masterGain.gain.exponentialRampToValueAtTime(
      normalizedVolume,
      Tone.now() + 0.1
    );
  ...
  public getVolume(): number {
    return this.state[this.activeTab].level;
  }
}
