...
function setSliders(mod: number, car: number, noi: number, iso: number, bin: number, bil: number, fm: number, lvl: number, g?: number): void {
    if (typeof g === "undefined") g = G;
    setSliderMod(mod, g);
    setSliderCar(car, g);
...
function msg(txt: string): void {
    ...
    msgTimer = setTimeout(function () { msg("<b class='mod'>In the Top Left Nav Bar, click on Learn</b> for more instructions...") }, 5000);
}

function newSet(setting: string): void {
    let mod: number, car: number, iso: number, bin: number, bil: number, fm: number, lvl: number;
    let fmin: number, fmax: number;
    mod = MOD[G]; car = CARRIER[G]; iso = ISOCHRONIC[G]; bin = BINAURAL[G]; bil = BILATERAL[G]; fm = FM[G]; lvl = LEVEL[G];
...
function muteOutput(): void {
    ...
    if (bMute) {
        ...
        for (let t = 0; t < TABS; t++) gainGlobal[t].gain.value = (LEVEL[t] * LEVEL[t]) / 10000;
    }
}
...
function showTopMenuButtons(): void {
    $("#hamburger").css('background-image', 'url("Pix/hamburger.png")');
